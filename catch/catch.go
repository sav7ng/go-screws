...
func (s *CatchTool) OnWriteErrToFile(errstring string) {
	path := s.GetModelPath() + "/err"
...
// 获取目录地址
func (s *CatchTool) GetModelPath() string {
...
func (s *CatchTool) PathExists(path string) bool {
	_, err := os.Stat(path)
	if err == nil {
		return true
	}
